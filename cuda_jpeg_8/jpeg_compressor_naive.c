...
int main(int argc, char *argv[]) {
	...
	app_timer_t start_dct, stop_dct;
	...
	timer(&start_dct);
	int16_t **y_data_out = dct_naive(image->Y, image->width, image->height, &number_of_dct_blocks, std_luminance_qt);
	int16_t **cb_data_out = dct_naive(image->Cb, image->width, image->height, &number_of_dct_blocks, std_chrominance_qt);
	int16_t **cr_data_out = dct_naive(image->Cr, image->width, image->height, &number_of_dct_blocks, std_chrominance_qt);
	timer(&stop_dct);

	printf("Discrete cosinus transform completed in %.3f ms\n", elapsed_time(start_dct, stop_dct));
